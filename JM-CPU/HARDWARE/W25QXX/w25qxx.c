...
{ 
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);//使能GPIOB时钟
	  ...
	  //GPIOB14
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;//PB14
...
//读取ReadAddr地址下一字节数据
//返回该地址下的值:				    
u8 W25QXX_Read_Dyte(u32 ReadAddr)
{
	u8 data;  
	W25QXX_CS=0;				    
	SPI2_ReadWriteByte(W25X_ReadData);         //发送读取命令   
	...
	SPI2_ReadWriteByte((u8)((ReadAddr)>>8));   
	SPI2_ReadWriteByte((u8)ReadAddr);   
	data=SPI2_ReadWriteByte(0XFF);   //循环读数 
	W25QXX_CS=1;
	return data;
}
...
//ReadAddr:开始读取的地址(24bit)
//NumByteToRead:要读取的字节数(最大65535)
void W25QXX_Read_Num(u32 ReadAddr,u16 Num)   
{ 
 	u16 i;   
	W25QXX_CS=0;	//使能器件   
    SPI2_ReadWriteByte(W25X_ReadData);         //发送读取命令   
    ...
    for(i=0;i<Num;i++)
	{ 
        Buffer[i]=SPI2_ReadWriteByte(0XFF);   //循环读数  
    }	
	W25QXX_CS=1	;
}  
