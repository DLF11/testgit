...
void LCD_Set(void);			//设置显示屏参数	 需要设置，显示功能都用到设置的．
void LCD_Init(void);		//初始化函数
void ILI9225G_Rst ( void ); //复位函数
void LCD_WriteReg(u16 LCD_Reg, u16 LCD_RegValue); //LCD_RegValue:要写入的数据
u16 LCD_RD_DATA(void);		//返回值:读到的值
void LCD_WR_REG(vu16 regval);	//写寄存器函数			
void LCD_Fast_DrawPoint(u16 x,u16 y,u16 color);//快速画点  modify YZ
void LCD_SetCursor(u16 Xpos, u16 Ypos);        //设置光标位置    MODIFY YZ
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode);//在指定位置显示一个字符   YZ
void LCD_WR_DATA(u16 data);  //16BIT REG
void LCD_WriteRAM(u16 RGB_Code);
void LCD_Display(u16 color);  //显示填充色
void LCD_DisplayJiuGongGe(void);  //九宫格显示:RGB-BGR-RGB
void Show_Gray(void) ;//灰阶显示
void LCD_Frame(u16 RGB_Data,u16 f_data);//四边线＋中间区域内填充单个颜色  
void DisplayPicture(void);//图片显示函数
